...
def test_archive_note_by_sliding_left():
    ...
    Slide.temp_bounds_swipe_left()      # bounds 기준 좌표 호출 못알아내서 임시 함수로 처리함
    time.sleep(2)

# test2) 우로 슬라이드
def test_archive_note_by_sliding_right():
    run_test_preconditions(1)
    Slide.temp_bounds_swipe_right()     # bounds 기준 좌표 호출 못알아내서 임시 함수로 처리함
    time.sleep(2)
...
# def test_archive_bottom_note_click():
#     run_test_preconditions(7)
#     # 화면 아래로 슬라이드
#     # 가장 마지막 생성된 test note 이름 확인
#     # 슬라이드하여 삭제
#
#
# # result 부분 조금 더 생각 필요함
# # result - Notes 에서 사라짐
# def result_del_notes(noteEle):
#     print(noteEle)
#     # 이동한 note 의 element 가 확인되지 않을 때 PASS
#
# # result - Archive 로 이동 확인됨
# # notes 에서 이동한 note title과 contents가 일치할 때 PASS else: FAIL
# def result_move_archive(noteTitle, noteContents):
#     note_title = get_element(xpath_notes_1_title)
#     notes_content = get_element(xpath_notes_1_content)
#     if note_title.text == noteTitle & notes_content == noteContents:
#         print("PASS")
#     else:
#         print("FAIL")
# #########################################################
